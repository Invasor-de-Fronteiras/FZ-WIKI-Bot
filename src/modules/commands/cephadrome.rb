module Bot::DiscordCommands
    module Monsters
      extend Discordrb::Commands::CommandContainer

      command :Cephadrome do |msg|
       # msg.respond  "https://static.wikia.nocookie.net/monsterhunter/images/4/44/1stGen-Cephadrome_and_Cephalos_Render_001.gif/revision/latest?cb=20131228064833"
        msg.respond  "https://cdn.discordapp.com/attachments/902336832069595197/905170068302864435/cephadrome.png"
    msg.respond "***Based on Cutting Damage!***
    **Name:** Cephadrome / ドスガレオス (Dosugareosu)
    **Title:** Sand Wyvern / 砂竜
    **Rank:** HR1-4
    **Weak to:** Ice <:zice:902635629471469638> and Thunder <:zthunder:902635597305372752>
    **Ailments:** Paralysis <:zpara:902638465680814161> and Waterblight <:zwater:902635559967686756>
    **Elements:** Water <:zwater:902635559967686756>
    **Weak Spots:** ```
    \n   Hitzone      Cut   Imp   Shot   Fire   Water   Thu   Dra   Ice   K.O  
    \n  ------------ ----- ----- ------ ------ ------- ----- ----- ----- ----- 
    \n   Head          40    50     60      0      10    15     5    30    80  
    \n   Neck         110   100    150      5      10    15     5    15     0  
    \n   Back         100    90    150      0      12    20     5    17     0  
    \n   Belly        120   130    100      0      15    15     5    20     0  
    \n   Tail          60    50     60      0      10    10     5    15     0  
    \n   Wings         40    50     60      0      10    10     5    15     0  
    \n   Legs          35    30     35      0      15    10     5    20     0   
    ```"
      end
      command :Cephadrome_HR do |msg|
       # msg.respond  "https://static.wikia.nocookie.net/monsterhunter/images/8/84/FrontierGen-HC_Cephadrome_Render_001.png/revision/latest?cb=20140817144054"
        msg.respond  "https://cdn.discordapp.com/attachments/902336832069595197/905170496524529664/cephadromehr5.png"
    msg.respond "***Based on Cutting Damage!***
    **Name:** Cephadrome / ドスガレオス (Dosugareosu)
    **Title:** Sand Wyvern / 砂竜
    **Rank:** HR5
    **Weak to:** Ice <:zice:902635629471469638> and Thunder <:zthunder:902635597305372752>
    **Ailments:** Paralysis <:zpara:902638465680814161> and Waterblight <:zwater:902635559967686756>
    **Elements:** Water <:zwater:902635559967686756>
    **Weak Spots:** ```
    \n   Hitzone      Cut   Imp   Shot   Fire   Water   Thu   Dra   Ice   K.O  
    \n  ------------ ----- ----- ------ ------ ------- ----- ----- ----- ----- 
    \n   Head          30    40     40     10       5    15     5     0    80  
    \n   Neck          20    20     10      5       5     5     5     0     0  
    \n   Back          50    50     20     -5       5    20     0    -5     0  
    \n   Belly         30    25     60     20       5    15     0    -5     0  
    \n   Tail          60    20     25     -5       5    15     5    25     0  
    \n   Wings         30    30     30     -5       5    15     5    15     0  
    \n   Legs          35    30     35     -5       5     0    15     5     0  
    ```"
      end
      command :Cephadrome_G do |msg|
       # msg.respond  "https://static.wikia.nocookie.net/monsterhunter/images/6/66/2ndGen-Cephadrome_Render_001.png/revision/latest?cb=20150303005421"
        msg.respond  "https://cdn.discordapp.com/attachments/902336832069595197/905170068302864435/cephadrome.png"
    msg.respond "***Based on Cutting Damage!***
    **Name:** Cephadrome / ドスガレオス (Dosugareosu)
    **Title:** Sand Wyvern / 砂竜
    **Rank:** G-Rank
    **Weak to:** Ice <:zice:902635629471469638> and Thunder <:zthunder:902635597305372752>
    **Ailments:** Paralysis <:zpara:902638465680814161> and Waterblight <:zwater:902635559967686756>
    **Elements:** Water <:zwater:902635559967686756>
    **Weak Spots:** ```
    \n   Hitzone      Cut   Imp   Shot   Fire   Water   Thu   Dra   Ice   K.O  
    \n  ------------ ----- ----- ------ ------ ------- ----- ----- ----- ----- 
    \n   Head          30    35     30      0      10    15     5    30    80  
    \n   Neck          60    65     70      5      10    15     5    15     0  
    \n   Back          50    50     70      0      12    30     5    17     0  
    \n   Belly         60    65     50      0      15    15     5    20     0  
    \n   Tail          30    25     30      0      10    10     5    15     0  
    \n   Wings         25    20     20      0      10    10     5    15     0  
    \n   Legs          20    25     30      0      15    10     5    20     0  
    ```"
      end
    end
end