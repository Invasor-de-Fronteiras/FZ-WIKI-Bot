module Bot::DiscordCommands
    module Monsters
      extend Discordrb::Commands::CommandContainer

      command :Black_Gravios do |msg|
        # msg.respond  "https://static.wikia.nocookie.net/monsterhunter/images/d/df/MHF1-Black_Gravios_Render_001.png/revision/latest?cb=20140112101418"
        msg.respond  "https://cdn.discordapp.com/attachments/902336832069595197/904788852672192622/blackgravios.png"
    msg.respond "***Based on Cutting Damage!***
    **Name:** Black Gravios / グラビモス亜種 (Gurabimosu Ashu)
    **Title:** Black Armor Wyvern / 黒鎧竜
    **Rank:** HR1-4
    **Weak to:** Water <:zwater:902635559967686756>
    **Ailments:** Poison <:zpoison:902638513097408513> and Sleep <:zsleep:902638414283821086>
    **Elements:** Fire <:zfire:902602768525717534>
    **Weak Spots:** ```
    \n                                Standard
    \n   Hitzone      Cut   Imp   Shot   Fire   Water   Thu   Dra   Ice   K.O  
    \n  ------------ ----- ----- ------ ------ ------- ----- ----- ----- ----- 
    \n   Head          20    30     20      0      15     5     0     0   100  
    \n   Neck          30    30     20      0      30     5     0     0     0  
    \n   Back          20    30     20      0      25     5     0     0     0  
    \n   Belly         15    25     20      0      35     5     0     0     0  
    \n   Tail          25    30     20      0      30     5     0     0     0  
    \n   Wings         20    20     20      0      20     5     0     0     0  
    \n   Legs          15    25     20      0      30     5     0     0     0       
    \n  ------------ ----- ----- ------ ------ ------- ----- ----- ----- ----- 
    \n                              Broken Chest									
    \n   Hitzone      Cut   Imp   Shot   Fire   Water   Thu   Dra   Ice   K.O 
    \n  ------------ ----- ----- ------ ------ ------- ----- ----- ----- ----- 
    \n   Head          20    30     20      0      15     5     0     0   200  
    \n   Neck          30    30     20      0      30     5     0     0     0  
    \n   Back          20    30     20      0      25     5     0     0     0  
    \n   Belly         80    80     75     10      50     5    30     0     0  
    \n   Tail          25    30     20      0      30     5     0     0     0  
    \n   Wings         20    20     20      0      20     5     0     0     0  
    \n   Legs          15    25     20      0      30     5     0     0     0    
    ```"
      end
      command :Black_Gravios_HR do |msg|
        # msg.respond  "https://static.wikia.nocookie.net/monsterhunter/images/d/d5/FrontierGen-HC_Black_Gravios_Render_001.png/revision/latest?cb=20140817142650"
        msg.respond  "https://cdn.discordapp.com/attachments/902336832069595197/904788852672192622/blackgravios.png"
    msg.respond "***Based on Cutting Damage!***
    **Name:** Black Gravios / グラビモス亜種 (Gurabimosu Ashu)
    **Title:** Black Armor Wyvern / 黒鎧竜
    **Rank:** HR5
    **Weak to:** Water <:zwater:902635559967686756>
    **Ailments:** Poison <:zpoison:902638513097408513> and Sleep <:zsleep:902638414283821086>
    **Elements:** Fire <:zfire:902602768525717534>
    **Weak Spots:** ```
    \n                                Standard
    \n   Hitzone      Cut   Imp   Shot   Fire   Water   Thu   Dra   Ice   K.O  
    \n  ------------ ----- ----- ------ ------ ------- ----- ----- ----- ----- 
    \n   Head          30    40     30      0      10     5     0     0   100  
    \n   Neck          40    40     30      0      30     5     0     0     0  
    \n   Back          30    40     35      0      20     5     0     0     0  
    \n   Belly         30    40     25      0      30     5     0     0     0  
    \n   Tail          35    40     30      0      30     5     0     0     0  
    \n   Wings         30    30     30      0      20     5     0     0     0  
    \n   Legs          25    35     30      0      30     5     0     0     0      
    \n  ------------ ----- ----- ------ ------ ------- ----- ----- ----- ----- 
    \n                              Broken Chest									
    \n   Hitzone      Cut   Imp   Shot   Fire   Water   Thu   Dra   Ice   K.O 
    \n  ------------ ----- ----- ------ ------ ------- ----- ----- ----- ----- 
    \n   Head          30    40     30      0      10     5     0     0   200  
    \n   Neck          40    40     30      0      30     5     0     0     0  
    \n   Back          30    40     35      0      20     5     0     0     0  
    \n   Belly         70    70     70      0      50     5    40     0     0  
    \n   Tail          35    40     30      0      30     5     0     0     0  
    \n   Wings         30    30     30      0      20     5     0     0     0  
    \n   Legs          25    35     30      0      30     5     0     0     0     
    ```"
      end
      command :Black_Gravios_G do |msg|
        # msg.respond  "https://static.wikia.nocookie.net/monsterhunter/images/4/44/MH4-Black_Gravios_Render_001.png/revision/latest?cb=20140105235345"
        msg.respond  "https://cdn.discordapp.com/attachments/902336832069595197/904788852672192622/blackgravios.png"
    msg.respond "***Based on Cutting Damage!***
    **Name:** Black Gravios / グラビモス亜種 (Gurabimosu Ashu)
    **Title:** Black Armor Wyvern / 黒鎧竜
    **Rank:** G-Rank
    **Weak to:** Water <:zwater:902635559967686756>
    **Ailments:** Poison <:zpoison:902638513097408513> and Sleep <:zsleep:902638414283821086>
    **Elements:** Fire <:zfire:902602768525717534>
    **Weak Spots:** ```
    \n                                Standard
    \n   Hitzone      Cut   Imp   Shot   Fire   Water   Thu   Dra   Ice   K.O  
    \n  ------------ ----- ----- ------ ------ ------- ----- ----- ----- ----- 
    \n   Head          20    30     20      0      15     5     0     0   100  
    \n   Neck          30    30     20      0      30     5     0     0     0  
    \n   Back          20    30     20      0      25     5     0     0     0  
    \n   Belly         20    25     20      0      45     5     0     0     0  
    \n   Tail          25    30     20      0      30     5     0     0     0  
    \n   Wings         20    20     20      0      20     5     0     0     0  
    \n   Legs          15    25     20      0      30     5     0     0     0      
    \n  ------------ ----- ----- ------ ------ ------- ----- ----- ----- ----- 
    \n                              Broken Chest									
    \n   Hitzone      Cut   Imp   Shot   Fire   Water   Thu   Dra   Ice   K.O 
    \n  ------------ ----- ----- ------ ------ ------- ----- ----- ----- ----- 
    \n   Head          20    30     20      0      15     5     0     0   200  
    \n   Neck          30    30     20      0      30     5     0     0     0  
    \n   Back          20    30     20      0      25     5     0     0     0  
    \n   Belly         80    80     70     10      60     5    30     0     0  
    \n   Tail          25    30     20      0      30     5     0     0     0  
    \n   Wings         20    20     20      0      20     5     0     0     0  
    \n   Legs          15    25     20      0      30     5     0     0     0  
    ```"
      end
    end
end