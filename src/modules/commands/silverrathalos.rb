module Bot::DiscordCommands
    module Monsters
      extend Discordrb::Commands::CommandContainer

      command :Silver_Rathalos do |msg|
        # msg.respond  "https://static.wikia.nocookie.net/monsterhunter/images/1/1b/MHF1-Silver_Rathalos_Render_001.png/revision/latest?cb=20150303050317"
        msg.respond  "https://cdn.discordapp.com/attachments/902336832069595197/906282943486521414/silverrathalos.png"
    msg.respond "***Based on Cutting Damage!***
    **Name:** Silver Rathalos / リオレウス希少種
    **Title:** Silver Fire Wyvern / 銀火竜
    **Rank:** HR1-4
    **Weak to:** Water <:zwater:883120128017965066> and Thunder <:zthunder:883120020312440852>
    **Ailments:** Stun <:stun:883125464397398106> and Poison <:zpoison:883122917976719451>
    **Elements:** Fire <:zfire:883120156916744312>
    **Weak Spots:** ```
    \n   Hitzone      Cut   Imp   Shot   Fire   Water   Thu   Dra   Ice   K.O
    \n  ------------ ----- ----- ------ ------ ------- ----- ----- ----- -----
    \n   Head          25    80     25      0      30    30     0    10   200
    \n   Neck          35    55     40      0      20    20     0     5     0
    \n   Back          20    25     40      5      20    20     0     5     0
    \n   Belly         35    45     50      0      15    15     0    10     0
    \n   Tail          25    25     70      0      10    10     0     5     0
    \n   Wings         80    20     30      0      30    30     0    10     0
    \n   Legs          25    40     50      0      10    10     0     5     0
    ```"
          puts ">

╔══════════════════════════════════════════════════════════════════════╗
║ MHFZ WIKI was summoned to: #{event.server.name} (#{event.server.id}) ║
╠══════════════════════════════════════════════════════════════════════╣
║ by #{event.author.username}                                          ║
║ in the reign of the King #{event.server.owner.name}                  ║
╚══════════════════════════════════════════════════════════════════════╝

<"
end
      command :Silver_Rathalos_HR do |msg|
        # msg.respond  "https://static.wikia.nocookie.net/monsterhunter/images/c/cc/FrontierGen-HC_Silver_Rathalos_Render_002.png/revision/latest?cb=20140423213326"
        msg.respond  "https://cdn.discordapp.com/attachments/902336832069595197/906283177444782172/silverrathaloshr5.png"
    msg.respond "***Based on Cutting Damage!***
    **Name:** Silver Rathalos / リオレウス希少種
    **Title:** Silver Fire Wyvern / 銀火竜
    **Rank:** HR5
    **Weak to:** Water <:zwater:883120128017965066> and Thunder <:zthunder:883120020312440852>
    **Ailments:** Stun <:stun:883125464397398106> and Poison <:zpoison:883122917976719451>
    **Elements:** Fire <:zfire:883120156916744312>
    **Weak Spots:** ```
    \n   Hitzone      Cut   Imp   Shot   Fire   Water   Thu   Dra   Ice   K.O
    \n  ------------ ----- ----- ------ ------ ------- ----- ----- ----- -----
    \n   Head          40    60     25      0       0    10    20    20   100
    \n   Neck          25    55     20      0       0     5    15    20     0
    \n   Back          20    25     40      5       0     5    30    30     0
    \n   Belly         30    35     30      0       0    10    20    15     0
    \n   Tail          45    45     50      0       0     5    30    10     0
    \n   Wings         60    20     20      0       0    10    10    30     0
    \n   Legs          25    40     30      0       0     5    15    10     0
    ```"
          puts ">

╔══════════════════════════════════════════════════════════════════════╗
║ MHFZ WIKI was summoned to: #{event.server.name} (#{event.server.id}) ║
╠══════════════════════════════════════════════════════════════════════╣
║ by #{event.author.username}                                          ║
║ in the reign of the King #{event.server.owner.name}                  ║
╚══════════════════════════════════════════════════════════════════════╝

<"
end
      command :Silver_Rathalos_G do |msg|
        # msg.respond  "https://static.wikia.nocookie.net/monsterhunter/images/3/39/MH4-Silver_Rathalos_Render_001.png/revision/latest?cb=20140116225450"
        msg.respond  "https://cdn.discordapp.com/attachments/902336832069595197/906282943486521414/silverrathalos.png"
    msg.respond "***Based on Cutting Damage!***
    **Name:** Silver Rathalos / リオレウス希少種
    **Title:** Silver Fire Wyvern / 銀火竜
    **Rank:** G-Rank
    **Weak to:** Water <:zwater:883120128017965066> and Thunder <:zthunder:883120020312440852>
    **Ailments:** Stun <:stun:883125464397398106> and Poison <:zpoison:883122917976719451>
    **Elements:** Fire <:zfire:883120156916744312>
    **Weak Spots:** ```
    \n   Hitzone      Cut   Imp   Shot   Fire   Water   Thu   Dra   Ice   K.O
    \n  ------------ ----- ----- ------ ------ ------- ----- ----- ----- -----
    \n   Head          25    60     25      0      20    25     0    10   200
    \n   Neck          35    40     25      0      15    15     0     5     0
    \n   Back          20    25     20      5      15    15     0     5     0
    \n   Belly         35    35     30      0      10    10     0    10     0
    \n   Tail          25    20     60      0       5    10     0     5     0
    \n   Wings         60    20     25      0      20    30     0    10     0
    \n   Legs          25    40     35      0       5    10     0     5     0
    ```"
          puts ">

╔══════════════════════════════════════════════════════════════════════╗
║ MHFZ WIKI was summoned to: #{event.server.name} (#{event.server.id}) ║
╠══════════════════════════════════════════════════════════════════════╣
║ by #{event.author.username}                                          ║
║ in the reign of the King #{event.server.owner.name}                  ║
╚══════════════════════════════════════════════════════════════════════╝

<"
end
    end
end